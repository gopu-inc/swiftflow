...
// ======================================================
// [SECTION] MAIN PARSING FUNCTIONS
// ======================================================
...
ASTNode* parse_print_statement(Parser* parser) {
    ...
    // Check for optional parentheses: print(...)
    ...
    // Create a list node to hold all arguments
    ASTNode* args_list = ast_new_node(NODE_LIST, line, column);
    ASTNode** next_arg = &args_list->left;

    // Handle empty print: print()
    if (has_parens && parser_check(parser, TK_RPAREN)) {
        parser_advance(parser);
        ASTNode* print_node = ast_new_node(NODE_PRINT, line, column);
        print_node->left = NULL; // No args
        return print_node;
    }

    // Parse at least one expression, then loop for commas
    do {
    ...
    if (has_parens) {
        parser_consume(parser, TK_RPAREN, "Expected ')' after print arguments");
    }
    
    ASTNode* print_node = ast_new_node(NODE_PRINT, line, column);
    print_node->left = args_list; // Attach list to left
...
static ASTNode* parse_class_declaration(Parser* parser) {
    ...
    if (parser_match(parser, TK_COLON)) {
        ...
        if (parent_token.kind != TK_ERROR) {
            parent = ast_new_identifier(str_ncopy(parent_token.start, parent_token.length), 
                                      parent_token.line, parent_token.column);
        }
    }
...
static ASTNode* parse_primary(Parser* parser) {
    ...
    if (parser_match(parser, TK_IDENT)) {
        ...
        if (parser_check(parser, TK_LPAREN)) {
            return parse_function_call(parser, node);
        }
        
        // Check for member access (e.g., obj.prop)
        if (parser_match(parser, TK_PERIOD)) {
            ...
            member_node->right = ast_new_identifier(str_ncopy(member.start, member.length), 
...
static ASTNode* parse_function_call(Parser* parser, ASTNode* callee) {
    // The previous token was '(', so the start of the call is the callee's line
    int line = callee->line;
    int column = callee->column;
    
    ASTNode* func_call = ast_new_node(NODE_FUNC_CALL, line, column);
    func_call->left = callee; // Function name/identifier
